...
class Wage_Responsivebannerslider_Adminhtml_ResponsivebannersliderController extends Mage_Adminhtml_Controller_action
{
    ...
    {
        $responsivebannersliderIds = $this->getRequest()->getParam('ids');
        if(!is_array($responsivebannersliderIds)) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('Please select item(s)'));
        } else {
            try {
                foreach ($responsivebannersliderIds as $responsivebannersliderId) {
                    $responsivebannerslider = Mage::getSingleton('responsivebannerslider/responsivebannerslider')
                        ->load($responsivebannersliderId)
                        ->setStatus($this->getRequest()->getParam('status'))
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($responsivebannersliderIds))
                );
            } catch (Exception $e) {
